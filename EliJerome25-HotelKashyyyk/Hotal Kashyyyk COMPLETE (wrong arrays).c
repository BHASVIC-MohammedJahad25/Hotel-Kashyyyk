...
char bookingIDs[20][6], boardName[4][6], guestNames[30][6], DoB[20][6], dailyNewspaper[6];
...
int roomnumber() {
    ...
    do {
        printf("Please enter room number (1-6), or 'quit' to return to the main menu:");
        fflush(stdin);
        scanf("%s",&tempinp);
...
void check_in() {
    ...
    do {
        ...
        if (strcmp(input, "NAME") == 0) {
            ...
            printf("Excellent!\nYour booking ID is %s", bookingIDs[temproomnum-1]);
            check = 0;
        }
    } while (check == 1);
    printf("\nCheck-In Complete.\n\n");
...
void handle_name(int roomnum) {
    char temp[20][6];
    srand(time(NULL));
    bookingIDs[0][roomnum] = '\0';
    int randomnum = rand() % 100 + 1;
    do {
        printf("Please enter your forename:");
        fflush(stdin);
        gets(guestNames[roomnum]);
        if (strcmp(guestNames[roomnum], " ") == 0 || strlen(guestNames[roomnum]) == 0) {
            printf("Not Accepted\n");
        }
    } while (strcmp(guestNames[roomnum], " ") == 0 || strlen(guestNames[roomnum]) == 0);
    strcat(guestNames[roomnum], " ");
    do {
        printf("Please enter your surname:");
        fflush(stdin);
        gets(temp[0]);
        if (strcmp(temp[0], " ") == 0 || strlen(temp[0]) == 0) {
            printf("Not Accepted\n");
        }
    } while (strcmp(temp[0], " ") == 0 || strlen(temp[0]) == 0);
...
void partysize(int roomnum) {
    do {
        ...
        if (adultGuests[roomnum]+childGuests[roomnum]>4) {
            printf("Your party size is too large - we only accept bookings of 4 or less people. If your party size is larger, consider booking two rooms.\nPlease re-enter your information.\n");
        }
        else if (adultGuests[roomnum]+childGuests[roomnum]<=0) {
            printf("Your party size is too small. You can't book for nobody!\nPlease re-enter your information.\n");
        }
    } while (adultGuests[roomnum]+childGuests[roomnum]>4 || adultGuests[roomnum]+childGuests[roomnum]<=0);
}
    ...
    if (IDvalid == 1) {
        if (boardType[index] == 20 || boardType[index] == 15) {
            ...
            while (Break == 0){
                ...
                printf("\nYour party size is %d, so please pick an option with %d or more free seats:", partysize, partysize);
                scanf("%d", &table);
                switch (table) {
                    case 1: {
                        if (endor[0] >= partysize) {
                            endor[0] -= partysize;
                            printf("Success, your table is booked at Endor for 7pm!\n\n");
                            Break = 1;
                        }
                        else {
                    ...
                    case 2: {
                        if (naboo[0] >= partysize) {
                            naboo[0] -= partysize;
                            printf("Success, your table is booked at naboo for 7pm!\n\n");
                            Break = 1;
                        }
                        else {
                            printf("Sorry, your party is too large for this table at this time");
                    ...
                    case 3: {
                        if (tattooine[0] >= partysize) {
                            ...
                            printf("Success, your table is booked at Tattooine for 7pm!\n\n");
                            Break = 1;
                        }
                        else {
                            printf("Sorry, your party is too large for this table at this time");
                    ...
                    case 4: {
                        if (endor[1] >= partysize) {
                            endor[1] -= partysize;
                            printf("Success, your table is booked at Endor for 9pm!\n\n");
                            Break = 1;
                        }
                        else {
                            printf("Sorry, your party is too large for this table at this time");
                    ...
                    case 5: {
                        if (naboo[1] >= partysize) {
                            naboo[1] -= partysize;
                            printf("Success, your table is booked at Naboo for 9pm!\n\n");
                            Break = 1;
                        }
                        else {
                            printf("Sorry, your party is too large for this table at this time");
                    ...
                    case 6: {
                        if (tattooine[1] >= partysize) {
                            ...
                            printf("Success, your table is booked at Tattooine for 9pm!\n\n");
                            Break = 1;
                        }
                        else {
                            printf("Sorry, your party is too large for this table at this time");
